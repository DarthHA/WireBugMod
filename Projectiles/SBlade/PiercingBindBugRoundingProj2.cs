...
{
    public class PiercingBindBugRoundingProj2 : ModProjectile
    {
        ...
        {
            float radian = Radian * MathHelper.Lerp(1, 0.6f, Math.Clamp(Projectile.ai[1] / 40f, 0, 1));
        ...
        public static void SummonProj(Vector2 Pos, Vector2 offset, Color color, float radian = 40, float rotateRadian = 0, float iniPhise = 0, float yModifier = 0.6f, float velocityModifier = 1, float lengthModifier = 1, int rotateDir = 1)
        {
            int protmp = Projectile.NewProjectile(Main.LocalPlayer.GetSource_FromThis(), Pos, Vector2.Zero, ModContent.ProjectileType<PiercingBindBugRoundingProj2>(), 0, 0, Main.myPlayer);
            if (protmp > -1)
            {
                PiercingBindBugRoundingProj2 modproj = Main.projectile[protmp].ModProjectile as PiercingBindBugRoundingProj2;
