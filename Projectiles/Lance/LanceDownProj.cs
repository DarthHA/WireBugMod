...
namespace WireBugMod.Projectiles.Lance
{
    ...
    public class LanceDownProj : BaseSkillProj
    {
        ...
        public override void AI()
        {
            ...
            if (Phase == LanceDownPhase.Shoot1)           //第一次发射翔虫,10帧
            {
                for (int i = 0; i < 2; i++)
                {
            ...
            else if (Phase == LanceDownPhase.Pause1)       //短暂暂停,20帧
            {
                Projectile.ai[1]++;
                ...
                else if (Projectile.ai[1] >= 5)    //翔虫回归
                {
                    BecomeTrail = true;
                    Projectile.velocity = Vector2.Normalize(owner.Center - Projectile.Center);     //为了兼容拖尾
            ...
            else if (Phase == LanceDownPhase.Shoot2)        //第二次发射翔虫,10帧
            {
                for (int i = 0; i < 2; i++)
                {
                    GenDust(owner.Center + new Vector2(Main.rand.Next(-10, 10), Main.rand.Next(-10, 10)), 0, 1 + Main.rand.NextFloat() * 0.5f);
            ...
            else if (Phase == LanceDownPhase.Pause2)       //短暂暂停,20帧
            {
                Projectile.ai[1]++;
