...
{
    public class LanceBugRoundingProj : ModProjectile
    {
        /// <summary>
        /// 环绕半径
        /// </summary>
        ...
        {
            int protmp = Projectile.NewProjectile(owner.GetSource_FromThis(), owner.Center, Vector2.Zero, ModContent.ProjectileType<LanceBugRoundingProj>(), 0, 0, owner.whoAmI);
            if (protmp > -1)
            {
                LanceBugRoundingProj modproj = Main.projectile[protmp].ModProjectile as LanceBugRoundingProj;
        ...
        public static void SummonProjFaster(Player owner, Vector2 offset, Color color, float radian = 40, float rotateRadian = 0, float iniPhise = 0, float yModifier = 0.6f, float velocityModifier = 1, float lengthModifier = 1, int rotateDir = 1)
        {
            int protmp = Projectile.NewProjectile(owner.GetSource_FromThis(), owner.Center, Vector2.Zero, ModContent.ProjectileType<LanceBugRoundingProj>(), 0, 0, owner.whoAmI);
            if (protmp > -1)
            {
                Main.projectile[protmp].extraUpdates = 1;
                LanceBugRoundingProj modproj = Main.projectile[protmp].ModProjectile as LanceBugRoundingProj;
                modproj.Radian = radian;
                modproj.RotateRadian = rotateRadian;
                modproj.IniPhase = iniPhise;
